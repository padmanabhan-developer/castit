...
// $sql = $sql . " ORDER BY m.profile_group_id " . $limit;
	...
	if(count($rows)>0) {
		...
		if(count($rows_image) > 0){
			...
			if(file_exists("/var/www/vhost/castit.dk/phpthumbnails/".$rows_image[$random_index]['image'])){
				// echo '<pre>';
				// echo $rows_image[$random_index]['image'];
				$profile_image = "https://castit.dk/phpthumbnails/".$rows_image[$random_index]['image'];
			}
			else{
