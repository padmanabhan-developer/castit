	...
	if(count($rows)>0) {
		...
		if(count($rows_image) > 0){
			foreach($rows_image as $row_image){
				...
				$profile_images[] = array('imgcnt' => $imgc, 'urloriginal' =>$row_image['image'], 'fullpath'=>'http://castit.dk/assets/profile_images/'.$path.$row_image['image']);
				$imgc++;
			}
		}
		...
		$response = array('success' => true, 'profile' => $profile_data, 'profile_images' => $profile_images, 'profile_videos' => $profile_videos, 'skills' => $skills, 'experiences' => $experiences, 'lang' => $lang );
			...
			if($rowcount > 0){
				...
				foreach($rows_lb_pprofiles as $rowp) {
						...
						$html .= '<td style="height:230px;width:139px" valign="top">
							<a href="http://ldsstage.in/castit/?l='.$lbid.'" target="_blank">
