...
$picture_table_query = $db->prepare("select * from photos where profile_id = $id AND media_slet_status = 0");
...
$video_table_query = $db->prepare("select * from videos where profile_id = $id AND media_slet_status = 0");
...
if($has_media){
  ...
  switch($mediatype){
      case 'images':
        $output .= form_images_html($value, $name, $profile_number);
        break;
      case 'videos':
        $output .= form_videos_html($value, $name, $profile_number);
        break;
      default:
